...
NSconvo_fit <- function( geodata = NULL, sp.SPDF = NULL,
  ...
  if( is.null(fixed.nugg2.var) == TRUE ){
    ...
    if( is.numeric(fixed.nugg2.var) ){ # Convert to a covariance matrix if specified as a vector
  ...
  if( !inherits(object, "NSconvo") ){
  ...
  else{
    {
      ...
      if( is.null(pred.fixed.nugg2.var) == TRUE ){
        ...
        if( is.numeric(pred.fixed.nugg2.var) ){ # Convert to a covariance matrix if specified as a vector
